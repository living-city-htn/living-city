...
describe('POST /api/posts/:id/like', () => {
  ...
  it('rejects a self-like without awarding either side', async () => {
    ...
    expect(balance(userId)).toBe(before)
    expect(likeCount(post.id)).toBe(0)
  })
  ...
  it('credits each side only once when a like is toggled off and back on', async () => {
    ...
    expect(await first.json()).toEqual({ liked: true, likes: 1, balance: likerBefore + 1 })
    expect(await second.json()).toMatchObject({ liked: false, likes: 0 })
    expect(await third.json()).toMatchObject({ liked: true, likes: 1 })
